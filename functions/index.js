...
async function processInstance(doc) {
    ...
    try {
        // Eligibility gates every mutation, including empty/deleted workflow completion.
        const sourceLead = await db.collection('canvas_leads').doc(instance.contactId).get();
        const instanceEligibility = communicationsPolicy.dispatchDecision(communicationsPolicy.configFromEnv(process.env), sourceLead.exists ? sourceLead.data() : null, instance, 'workflow');
        if (!instanceEligibility.allowed) {
            console.error('communication_refused', { instanceId, reason: instanceEligibility.reason });
            return; // leave ineligible work byte-for-byte unchanged
        }
