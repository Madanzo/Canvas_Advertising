...
test('forwarding readiness fails closed until every deployment input is confirmed', () => {
    ...
    assert.equal(readiness({ enabled: true, baseUrl: 'https://crm.example', tenantSlug: 'canvas', credential: 'mk_live_key.secret' }).reason, 'service-allowlist-unconfirmed');
    // A missing Secret Manager binding must refuse, not send unauthenticated.
    assert.equal(readiness({ enabled: true, baseUrl: 'https://crm.example', tenantSlug: 'canvas', credential: '' }).reason, 'credential-missing');
});
