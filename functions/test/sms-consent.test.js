...
function harness(current, grant) {
    ...
    const context = {smsConsent:policy, db, console:{log(){},error(){}}, admin:{firestore:{FieldValue:{serverTimestamp:()=> 'timestamp'}}}, sendSMS:async()=>{calls.sends++;return {success:true};},sendEmail:async()=>({success:true})};
...
test('shared sender also blocks direct SMS before provider/config access without explicit consent',async()=>{
 ...
 for(const value of [true,false,undefined]) {
  ...
  const context={smsConsent:policy,console:{warn(){},log(){},error(){}},db:{collection:()=>({doc:()=>({get:async()=>({exists:true,data:()=>lead(value)})})})},
