...
/** The purpose the engine itself resolves for wf_welcome's step N. */
const purposeAt = (index) => communicationsPolicy.stepPurpose('form_submit', WF_WELCOME[index], 'wf_welcome', index);
...
function harness(env) {
    ...
    return { context, calls, lead,
        instance: (stepIndex) => ({
            ...
            workflowId: 'wf_welcome', contactId: 'lead_1', communicationOrigin: 'form_submit',
...
test('step 0 — the lead acknowledgement — is suppressed, and does NOT fail the instance', async () => {
    ...
    const result = await h.context.execute(WF_WELCOME[0], h.instance(0), purposeAt(0));
    assert.equal(result.skipped, true);
    assert.equal(purposeAt(0), 'lead_received');
...
test('step 1 — the acknowledgement SMS — is suppressed too', async () => {
    const h = harness(CRM_OWNS_LEAD_RECEIVED);
    const result = await h.context.execute(WF_WELCOME[1], h.instance(1), purposeAt(1));
    assert.equal(result.skipped, true);
    assert.equal(purposeAt(1), 'lead_received');
...
test('STEP 2 — THE TWO-DAY FOLLOW-UP — STILL SENDS', async () => {
    ...
    const h = harness(CRM_OWNS_LEAD_RECEIVED);
    const result = await h.context.execute(WF_WELCOME[2], h.instance(2), purposeAt(2));
    assert.equal(result.skipped, undefined);
    assert.equal(h.calls.emails.length, 1);
    assert.equal(h.calls.emails[0].templateId, 'follow_up_no_response');
    assert.equal(h.calls.emails[0].options.purpose, 'follow_up');
    assert.equal(purposeAt(2), 'follow_up');
});
...
test('with the CRM owning nothing, every step sends as before', async () => {
    ...
    assert.equal((await h.context.execute(WF_WELCOME[0], h.instance(0), purposeAt(0))).skipped, undefined);
    assert.equal((await h.context.execute(WF_WELCOME[2], h.instance(2), purposeAt(2))).skipped, undefined);
    assert.equal(h.calls.emails.length, 2);
});

test('rollback does NOT re-send a receipt the CRM already owned', async () => {
    // The lead was stamped `crm` at capture. Rolling the configuration back
    // must not make the website send that receipt after the fact: the stamp,
    // not the current configuration, decides who owned it.
    const h = harness(CRM_OWNS_LEAD_RECEIVED);
    const rolledBack = { ...CRM_OWNS_LEAD_RECEIVED, CANVAS_CRM_OWNED_PURPOSES: '[]' };
    h.context.process.env = rolledBack;
    const result = await h.context.execute(WF_WELCOME[0], h.instance(0), purposeAt(0));
    assert.equal(result.skipped, true, 'a CRM-owned receipt stays the CRM\'s after rollback');
    assert.equal(h.calls.emails.length, 0);
    // Nothing is swept, backfilled or re-sent by the rollback itself.
    assert.equal(h.calls.updates.length, 0);
});

test('rollback DOES restore later website-owned steps, which never stopped anyway', async () => {
    const h = harness({ ...CRM_OWNS_LEAD_RECEIVED, CANVAS_CRM_OWNED_PURPOSES: '[]' });
    await h.context.execute(WF_WELCOME[2], h.instance(2), purposeAt(2));
    assert.equal(h.calls.emails.length, 1);
    assert.equal(h.calls.emails[0].templateId, 'follow_up_no_response');
});
