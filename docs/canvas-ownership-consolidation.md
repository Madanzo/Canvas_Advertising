...
## Concurrent consolidation reconciled

While this work ran, the CRM agent published `5cef7175e429a66760bb4b22defa0742164b07b6` on codex/canvas-ownership-canonical, a merge of d77 and ca915. This final review branch incorporates that commit as a second parent; both original histories are now reachable. No main merge or original-branch rewrite occurred. The .gitignore symlink fix is adopted.

5cef's immutable owner/configuration decisions match this branch. Its runtime differs in using workflow position before template and permitting declared purpose refinements, and it lacks the new eligibility/recovery checks. After source comparison, this branch deliberately retains its exact type/template map and no step.purpose authority. This avoids classifying a reordered follow-up as a receipt and covers the saved additional booking reminder. Its separate step-purpose module and 16 older tests remain available in parent history, not as an unused competing implementation in this tree. Their meaningful owner/rollback/per-step scenarios are covered by the active purpose/consolidation suites; their positional or permissive assertions are superseded. The final head is the sole proposed Canvas review successor; canonical CRM remains independently owned.
