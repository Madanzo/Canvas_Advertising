...
The proposed worker authorizes zero current message steps: all lack eligibility grants. For the 104 no-step definitions, processInstance marks the instance completed before the eligibility branch; worker resumption therefore is not read-only even when provider calls remain zero. This is a resumption gate, not silently treated as authorization. No records were modified in this inventory.
...
This change adds deployment tooling/tests/docs; no Functions runtime source, frontend, App Check, service mapping, payload, key or timestamp logic changed.
...
Excluded: syncLeadToCRM v6, createLeadUploadSession v3, all other Functions, all Hosting assets (including the five deployed frontend files/App Check), rules, indexes, CRM source changes, credentials, sends and replay. Runtime package bytes remain identical to the reviewed 75bf overlays; wrappers add only the deployment gate and proposed environment value. Existing legacy-trigger and SMS safeguards are preserved.
...
83 Functions unit tests passed, including 3 new deployment-precondition tests (missing/malformed/rotated values, consistency, preserved grants, pagination, read failure). Reused existing proof issuance, transaction/concurrency, retry and immediate onNewLead suppression tests: 11 split-overlay runtime tests passed against local demo Firestore, external providers mocked. No duplicate suppression assertion added. Split-overlay byte/dependency verifier passed. Exact-head CI must be reported separately after the review commit is published; earlier 75bf CI is not evidence for this new head.
