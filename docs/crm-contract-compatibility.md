...
## Blocking canonical route finding at the pinned SHA

`app/api/v1/tenants/[tenantSlug]/leads/intake/route.ts` calls `executeIntake` with raw/validated body and server time, but does **not** supply `communicationsPolicy` or `communicationsEnvelope`. `lib/leads/server/intake.ts` passes the absent policy as null to `decideEnrollment`; enrollment therefore remains `policy_missing`. The compatibility tests above deliberately execute the validator and enrollment contract with explicit fixture policy/envelope and do not claim HTTP route wiring is complete. The canonical CRM agent must bind tenant-owned policy and strictly parsed envelope to the authenticated intake transaction, and test that route, before any activation. No CRM code was edited here.
